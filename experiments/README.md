...
> - **bench-004** (86-file fixture, filename-obvious task): scaling alone didn't make the agent reach for PV. `without-pv` runs at 9 tools / 25s; coerced PV usage added overhead with no benefit. The directly-routed value remained unmeasured.
> - **bench-005** (86-file fixture + cross-domain hidden link): the routing value finally surfaces. Task — "when a subscription is cancelled, record an analytics event and queue a churn-risk email" — encodes the cancel→analytics+notif relation in the graph but not in filenames. `without-pv`: **44 tools / 103.5s / $0.225**. `with-pv-forced`: **20.5 tools (−53%) / 82s (−21%) / $0.192 (−15%)**. `with-pv-v3` was bimodal (the agent voluntarily invoked `pv ask` in 1 of 2 runs — the first organic PV usage seen across five benches). Same coercion that was pure overhead in bench-004 is net positive here; the variable is the *task shape*, not the coercion.
> - **bench-006** (4 synthetic PRD ↔ graph fixtures with planted drift): does `pv prd check --prompt` actually catch drift when an LLM follows the prompt? **4/4 produced an actionable signal, 3/4 used the correct output category, 0/4 false positives.** The one mismatch (D — unlinked-but-relevant Intent) is a known prompt-context limitation: the agent can only flag drift it can see, and unlinked nodes aren't in the section's context window. ~$0.05 per scenario via Claude.
> - **bench-007** (4 synthetic PR diffs with planted intent-layer drift): does the new `pv review --prompt` actually catch drift between code changes and the intent layer? **4/4 hit on the expected drift category; 2/4 caught additional real drift the test design didn't pre-specify; 0/4 false positives on the aligned control.** Total cost ~$0.30. Also measured Phase-A compression on PV's own self-review prompt: 129KB → 93KB (-27.9%) via PRD-section dedup + diff-cap + description truncation. First empirical data on the new "intent-drift gate" positioning.
> - **audit-after-pivot** (not a controlled bench — a *real audit run* on PV's own five-commit framing pivot): Layer 1 (validate / health / prd check / `pv changed`) clean; Layer 3 PRD ↔ graph clean across all 18 sections; Layer 3 code ↔ intent surfaced **1 small honest catch** (`API-PV-REVIEW` missing `uses → ENT-PV-CODEMAP`). Total cost ~$0.22 / 2 minutes. The audit doc records the process so future maintainers can see what was checked, what passed, what was caught — the kind of artifact PV was designed to produce.
>
> So PV's three values now have separate empirical anchors: **framing** (bench-002 — task-dependent), **routing tools** (bench-005 — needs cross-domain hidden links), **documentation** (all benches, agent-independent). And Layer 3 PRD drift detection has its own anchor in **bench-006 — works on the cases it's designed for, with honest limits documented.** See [`bench-003/README.md`](bench-003/README.md) for the experiment that broke the original "agent uses PV" story, [`bench-005/README.md`](bench-005/README.md) for routing value, and [`bench-006-prompt-quality/README.md`](bench-006-prompt-quality/README.md) for Layer 3 drift detection.
