...
## Section 1/1: User story: signing in

### PRD content

As a returning user, I want to enter my email and password to access my
account. The system verifies my credentials against the stored password
hash and issues a session token I can use for subsequent requests.

### Linked Intents

- **REQ-AUTH-001** — Users can sign in with email and password
  - description: Standard credential-based authentication. Email + password verified against a stored hash; on success a session token is issued.
  - tags: auth, login
- **API-AUTH-LOGIN** — POST /auth/login _(neighbor)_
  - description: Accepts email + password, returns a session token on success. Implements REQ-AUTH-001.
  - tags: auth, api
  - codemap: src/auth/login.ts
  - outgoing: implements → REQ-AUTH-001; uses → ENT-AUTH-USER
- **ENT-AUTH-USER** — User _(neighbor)_
  - description: User record: id, email, password_hash, created_at.
  - tags: auth, entity
  - codemap: src/auth/user.ts
