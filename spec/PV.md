...
**Domain entities — what PV operates on**

```mermaid
graph LR
  Source{{"24 APIs · 8 workflows · 16 requirements"}}
  n_ENT_PV_NODE["<b>SpecNode + Relation + Graph types</b><br/>ENT-PV-NODE"]
  Source -. "28×" .-> n_ENT_PV_NODE
  n_ENT_PV_CODEMAP["<b>CodeMap (node id → file paths)</b><br/>ENT-PV-CODEMAP"]
  Source -. "13×" .-> n_ENT_PV_CODEMAP
  n_ENT_PV_IMPACT_RESULT["<b>ImpactResult</b><br/>ENT-PV-IMPACT-RESULT"]
  Source -. "2×" .-> n_ENT_PV_IMPACT_RESULT
  n_ENT_PV_PARSED_PRD["<b>ParsedPrd</b><br/>ENT-PV-PARSED-PRD"]
  Source -. "1×" .-> n_ENT_PV_PARSED_PRD
  n_ENT_PV_OUTPUT["<b>CLI output helpers</b><br/>ENT-PV-OUTPUT"]
  Source -. "0×" .-> n_ENT_PV_OUTPUT
```
...
```mermaid
graph TB
  n_REQ_PV_003["<b>REQ-PV-003</b><br/>CLI is the only control surface<br/><i>cited 13×</i>"]
  n_REQ_PV_002["<b>REQ-PV-002</b><br/>Graph is the source of truth; markdown is a regenerated view<br/><i>cited 4×</i>"]
  n_REQ_PV_012["<b>REQ-PV-012</b><br/>Delegate LLM-shaped work to the user's coding agent via prompt templa…<br/><i>cited 4×</i>"]
  n_REQ_PV_015["<b>REQ-PV-015</b><br/>Strengthen the documentation-first positioning with diagrams, reverse…<br/><i>cited 4×</i>"]
  n_REQ_PV_014["<b>REQ-PV-014</b><br/>Give users a numerical handle on their own PV usage<br/><i>cited 3×</i>"]
```

_For the full 53-node graph: `pv diagram --domain PV -f mermaid`._
...
### [`API-PV-CHANGED`](API-PV-CHANGED.md) — pv changed [base]
...
### [`API-PV-REVIEW`](API-PV-REVIEW.md) — pv review [base] --prompt
