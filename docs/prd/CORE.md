...
`pv prd check`. It's the single canonical answer to "what is PV,
why does it exist, what is and isn't in scope" — readable by a
contributor before their first PR, by a skeptic before adopting it,
and by the maintainer six months from now when they've forgotten
...
2. **People.** New hires spend weeks ramping; senior departures take
...
3. **AI agents.** An agent has no Slack history, no senior to ping,
   ...
   hallucination or token-burning exploration.
...
and Confluence pages, and that worked while the only consumers were
people. AI coding agents change the calculation: the cost of keeping
intent in human memory was always there, agents make it visible.
...
- **Catch drift at PR time.** When code changes, surface the
  intent-layer connections it touches and the gaps it introduces:
  files added without a codemap entry, files removed while still
  referenced, behaviors changed without an Intent description update.
  `pv changed` is the structural gate; `pv review --prompt` is the
  semantic companion.
...
- Expose a **graph-integrity surface** that a CI run can gate on:
...
- An LLM API consumer. PV emits structured prompts (`pv generate
  ...
  `pv review --prompt`) that the user runs through their own coding
  agent. PV doesn't manage API keys, doesn't pick a model, doesn't
  pay for tokens.
...
### As a PR author, I see drift before I push.

`pv changed origin/main` reports orphan files, broken codemap
entries, and Intent nodes whose linked PRD sections may need updates
— in one structured pass over the diff. Exits 1 on warn/error so a
git hook or CI step can gate the push.

For deeper review when the diff is non-trivial: `pv review
origin/main --prompt` emits a Markdown prompt my coding agent reads.
The agent proposes intent-description updates, new Intent nodes for
shipped capabilities, and PRD section corrections; I review each
patch and apply via existing graph-mutation commands.

<!-- pv-intents: API-PV-CHANGED, API-PV-REVIEW -->

### As a code reviewer, I see graph-level changes in PRs.
...
on `has_breaking`. `pv changed` runs alongside it as the file-level
drift gate.
...
`pv ask "<intent>"` is an optional one-shot preamble: it classifies
the intent, queries the graph, and runs `impact` on the top hit. Use
it when starting from a free-form intent rather than a known node id.
...
PV's value lands in three places, in order of who sees it first:

1. **Drift caught at PR time.** `pv changed` fires on orphan files,
   broken codemap entries, and unlinked-but-relevant Intent nodes
   whose PRD sections may need updates. Self-host evidence: PV's own
   PR pipeline reports clean runs (0 orphans, 0 broken codemap)
   because the graph is updated alongside the code in each commit.
   The drift gate's value comes from *preventing* divergence rather
   than from one-off catches; the empirical anchor is bench-006
   (Layer 3 PRD ↔ graph drift, 4/4 actionable signals on planted
   drift) and bench-007 (`pv review --prompt`, 4/4 hits with 0/4
   false positives on the aligned control).

2. **Living architecture documentation.** The graph + `spec/` +
   `validate` + `health` + `diff` + `diagram` apply to every
   adopting team regardless of agent use. Self-host evidence: PV's
   own graph has 50+ nodes, 0 validate errors, all diagrams
   regenerate cleanly in CI. New contributors read `spec/README.md`
   for the index; PR reviewers see graph diffs in human-readable
   form.

3. **Focused context for AI agents.** `pv impact <id>` returns the
   asymmetric file set affected by a node change. The `coverage`
   indicator (narrow / broad / global) tells the agent whether to
   trust the set or also grep. Empirically, agents using PV-routed
   file sets read less defensively on cross-domain tasks where the
   right files are encoded in graph relations rather than filenames.

The benches under [`experiments/`](../../experiments/README.md) are
reproducible — each has its own `setup-fixture.sh` and `run.sh`.

PV ships `pv stats`, which aggregates the user's own usage from
`.polaris/usage.jsonl` so they can see their own numbers rather than
trusting ours.
...
- **Multi-repo intent unification** — one `.polaris/graph.json` per
  repo. Cross-repo intent is an organizational problem this tool
...
These remain unresolved as of the current ship; the answer will come
from observing actual adoption rather than guessing now.
...
- **Multi-file PRDs (Level 5 of the onboarding gradient)** — worth
  the complexity? Real demand from teams with very large PRDs vs
  theoretical scaling concern.
- **`pv review --prompt` cost on huge PRs.** Prompt size scales with
  diff size + linked nodes + PRD section count. For 50+ file PRs the
  prompt can hit 50K-200K tokens. A chunked-review architecture
  (per-section prompts) is the likely answer for huge codebases but
  hasn't been built.
- **`pv changed` automation.** A GitHub Action that runs `pv changed`
  on every PR and posts findings as a comment is the natural next
  step but unbuilt.
...
- **PRD layer Phase 2** — `template`, `decompose --prompt`, `lint`.
  Triggered by adoption signal (issues asking for it) rather than a
  calendar date.
- **GitHub Action for `pv changed`** — automate the structural drift
  gate on every PR with a comment-bot.
- **Chunked `pv review` for huge PRs** — per-section prompts that
  fit comfortably in any model's working window, aggregated into one
  report.
- **Multi-file PRDs** — natural extension of Phase 2's `decompose`
  if it surfaces patterns where one file is too coarse.
- **Editor integrations** — VS Code extension that highlights Intent
  IDs as links into `spec/<id>.md`. Not core; would be a separate
  package.
...
This document is the anchor: when scope-creep tempts a feature, when
a contributor asks "is X a PV concern?", when the maintainer is six
months and ten conversations away from this point in time — read this.
The Intent graph carries the *architecture*; this PRD carries the
*thesis*.
